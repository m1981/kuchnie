...
| **`session-gates.d/60-arch-smells.sh`** | end of session | nothing yet — WARN-only on NEW findings vs the accepted `docs/arch-smells-baseline.txt` (deferrals carry work-item ids): eleven smell classes (import cycles incl. deferred but excl. TYPE_CHECKING-guarded, cross-module `_underscore` imports, repeated deferred imports, dormant classes, god classes, duplicate module-level defs, unit-suffix lint, param bloat, entity-service imports, layer rules, stringly-enum) — the mechanical layer of the signature review (`docs/pattern-conformance.md` § Re-running this review) |
| **`session-gates.d/62-vocab-drift.sh`** | end of session | nothing yet — WARN-only: a NEW identifier-like literal shared across ≥3 modules of one package vs the committed `docs/shared-literals-baseline.txt` (25 accepted entries); grow the vocabulary deliberately via `scripts/shared-literals.py --write` |
