...
| **`session-gates.d/60-arch-smells.sh`** | end of session | nothing yet — WARN-only: six smell classes (import cycles incl. deferred, cross-module `_underscore` imports, repeated deferred imports, dormant classes, god classes, duplicate module-level defs) — the mechanical layer of the signature review (`docs/pattern-conformance.md` § Re-running this review) |
