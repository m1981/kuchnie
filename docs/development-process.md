...
| **`session-gates.d/60-arch-smells.sh`** | end of session | nothing yet — WARN-only: eight smell classes (import cycles incl. deferred, cross-module `_underscore` imports, repeated deferred imports, dormant classes, god classes, duplicate module-level defs, unit-suffix lint, param bloat) — the mechanical layer of the signature review (`docs/pattern-conformance.md` § Re-running this review) |
