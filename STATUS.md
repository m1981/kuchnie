...
> Generated: 2026-07-17T00:03:24+00:00
...
| purchasing-variants | — | Supplier price rows enter through the landing schema with verbatim sou… | LIVE | tr-d0610cb4 (live) |
...
- **1 retraction(s) ready** (human-only; every diverged claim's verdict names its live successor). Paste:
  ...
  ```
  for id in tr-747adf10; do TRUTH_HUMAN=1 TRUTH_HUMAN_ACK=$id scripts/truth verdict $id retracted --basis "superseded; successor in verdict trail"; done
  ```
...
- **3 claim(s) for an agent verifier** (not you — but only you start sessions). Move the needle: say *“dispatch a verifier sweep”*.

## 3 · What's next — ready lane, product | process

**Open 6 product / 6 process · closed 7d 20 product / 23 process (30 classified by keyword) ⚠ swing back to product**
...
| — | wk-68b32f3b | Wire assess_quote_freshness into the UI quote path: total_price in ui/state.py renders an estimate-grade marker with per-line age when any material price is stale or provenance-free | live |
...
**⚠ unmapped open bd issues** (both views above lie by omission until mapped): kuchnie-h9h, kuchnie-qfq — move the needle: add a row with stage+uc+axis to `docs/roadmap-map.csv`.

## 5 · Is the machine OK? — regime health

| Signal | State |
|---|---|
| spec-health | 🟢 spec-health: 0 failure(s), 45 warning(s) across 16 spec(s) |
...
| claims | cannot_verify 2 · diverged 1 · live 75 · retracted 39 · unverified 3 |
| verdict queue | 1 human (§ 2) + 0 verifier |
| backward trace (R2) | TRACED 39 · MENTIONED 33 · DARK 0 |
