...
> Generated: 2026-07-14T23:18:00+00:00

## Health

| Signal | State |
|---|---|
| spec-health | 🟢 spec-health: 0 failure(s), 38 warning(s) across 14 spec(s) |
| doc-health | 🟢 doc-health: 0 failure(s) across 89 live doc(s) |
| flagship baseline | guarded by `scripts/exercise-gate.sh` (runs at session-close) |
| claims | diverged 14 · live 30 · retracted 8 · stale 17 · unverified 10 |
...
| P2 | wk-0d7a80d2 / kuchnie-xdc | test-health satellite (R4/ISO 29119): sweep test-cited tr-/wk- ids against the ledger + session gate 40-test-health | warn: tr-7e0b27d6: premise not yet independently verified |
...
| — | wk-ea10d199 | Ledger hygiene: claims watching generated STATUS.md restale perpetually — supersede with source-only paths | HELD — tr-4f3217dc (stale) |
...
flowchart LR
    subgraph s0 ["0. Process/infra"]
        kuchnie_xdc["P2 kuchnie-xdc<br/>test-health satellite R4 (inc 1)"]
        kuchnie_7ai["P2 kuchnie-7ai<br/>Dashboard Completeness R7 (inc 2)"]
        kuchnie_31x["P2 kuchnie-31x<br/>Backward-trace R2-lite (inc 3)"]
        kuchnie_82w["P2 kuchnie-82w<br/>Upstream drafts, draft-only (inc 4)"]
    end
...
flowchart LR
    ...
    subgraph g_none ["no UC (process/infra — route or leave)"]
        ...
        u_kuchnie_xdc["P2 kuchnie-xdc<br/>test-health satellite R4 (inc 1)"]
        u_kuchnie_7ai["P2 kuchnie-7ai<br/>Dashboard Completeness R7 (inc 2)"]
        u_kuchnie_31x["P2 kuchnie-31x<br/>Backward-trace R2-lite (inc 3)"]
        u_kuchnie_82w["P2 kuchnie-82w<br/>Upstream drafts, draft-only (inc 4)"]
...
Unmapped open bd issues (add to `docs/roadmap-map.csv`): kuchnie-dry
