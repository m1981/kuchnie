...
> Generated: 2026-07-30T08:42:22+00:00
...
**Acceptance gauge (R7, all specs): 20/33 LIVE.** Denominator = pre-written intent, numerator = demonstrated fact; it can go DOWN when a commit stales a completion claim.
...
- **2 retraction(s) ready** (human-only; every diverged claim's verdict names its live successor). Paste:
  ...
  ```
  for id in tr-ec698a92 tr-cb1dec8a; do TRUTH_HUMAN=1 TRUTH_HUMAN_ACK=$id scripts/truth verdict $id retracted --basis "superseded; successor in verdict trail"; done
  ```
...
**Open 6 product / 9 process · closed 7d 4 product / 2 process (4 classified by keyword)**
...
Move the needle: `scripts/truth start wk-593a317b` · `bd update kuchnie-ubc --claim` — then say *“continue Purchasing artifacts: board-order + hard…”*
...
**⚠ unmapped open bd issues** (both views above lie by omission until mapped): kuchnie-3ue, kuchnie-60t, kuchnie-a44, kuchnie-bf0, kuchnie-mbn, kuchnie-wcj — move the needle: add a row with stage+uc+axis to `docs/roadmap-map.csv`.
...
| doc-health | 🟢 doc-health: 0 failure(s) across 116 live doc(s) |
| flagship baseline | guarded by `scripts/exercise-gate.sh` (runs at session-close) |
| claims | cannot_verify 1 · diverged 2 · live 123 · retracted 62 |
