...
> Generated: 2026-08-01T18:14:53+00:00
...
**Acceptance gauge (R7, all specs): 22/34 LIVE.** Denominator = pre-written intent, numerator = demonstrated fact; it can go DOWN when a commit stales a completion claim.
...
| purchasing-variants | — | Per-dealer hardware order CSVs emit net top-up quantities (required mi… | LIVE | tr-611a8240 (live) |
...
| height-parameter-set | — | validate_rows consumes a supplied height parameter set and G1 reports … | LIVE | tr-0d819e45 (live) |
...
- **4 retraction(s) ready** (human-only; every diverged claim's verdict names its live successor). Paste:

  ```
  for id in tr-ff8a5110 tr-06522739 tr-7855c415 tr-4d916887; do TRUTH_HUMAN=1 TRUTH_HUMAN_ACK=$id scripts/truth verdict $id retracted --basis "superseded; successor in verdict trail"; done
  ```
...
**Open 5 product / 11 process · closed 7d 4 product / 3 process (5 classified by keyword)**
...
Move the needle: `scripts/truth start wk-6716e9c8` · `bd update kuchnie-58o --claim` — then say *“continue Acquire missing decor miniature images (…”*

### PROCESS

| P | Work | Title | Premise health |
|---|---|---|---|
| P3 | wk-a898481e / kuchnie-b30 | DrawerBoxSpec parameter object: decompose_drawer_box (11/10 params) and make_runner_accessory (8) collapse their drawer-box arguments into one spec consumed by the DrawerSystem ABC, catalog decomposer and variant_derivation | live |
...
| P3 | wk-b669f4f5 / kuchnie-hij | Split the ERP god-states: AdminState (36 methods) and KitchenState (32) — extract row-layout math into a testable core module, group setters into substates; add_module (12 params) gets a module spec | live |
| — | wk-075803aa | Degenerate TURNS mapping in validator.py check_run_continuity: left and right turns map to the same next direction per from-direction (east+left and east+right both give south), so a geometrically-wrong turn label passes the direction check -- fix the mapping to true plan-geometry semantics in lockstep across validator, l-layout-model spec invariant 3, and adapter r4 extraction | live |
...
| — | wk-8908bacc | Migrate 11 rg-evidence claims to grep (ADR-040 aftermath: tr-de2e32ab tr-d053a05c tr-0c33b232 tr-7bed9678 tr-4d1d4ba9 tr-66f75050 tr-848f3f36 tr-43ab6d49 tr-88fd9681 tr-df1e70db tr-5a94302e cannot recheck with rg off the allowlist) | live |
| — | wk-a1898db5 | Selective deep traceability: claim-per-external-constant (Blum/edging/supplier dims, ~40 constants) and a branch-coverage floor gate on kuchnie-core as the condition-level proxy trace (DO-178C-style coverage-not-links) | live |
...
**⚠ unmapped open bd issues** (both views above lie by omission until mapped): kuchnie-15c, kuchnie-3ue, kuchnie-60t, kuchnie-a44, kuchnie-bf0, kuchnie-mbn, kuchnie-wcj, kuchnie-xjm — move the needle: add a row with stage+uc+axis to `docs/roadmap-map.csv`.

## 5 · Is the machine OK? — regime health

| Signal | State |
|---|---|
| spec-health | 🟢 spec-health: 0 failure(s), 57 warning(s) across 21 spec(s) |
| doc-health | 🟢 doc-health: 0 failure(s) across 119 live doc(s) |
| flagship baseline | guarded by `scripts/exercise-gate.sh` (runs at session-close) |
| claims | cannot_verify 1 · diverged 4 · live 124 · retracted 66 |
| verdict queue | 4 human (§ 2) + 0 verifier |
| backward trace (R2) | TRACED 44 · MENTIONED 32 · DARK 0 |
| R4 tests | `scripts/test-health.sh` — cited ids swept at close |
| R7 acceptance | 22/34 LIVE (§ 1) |
| last exercise run | 2026-07-13T14:38:15+00:00 · Blender 5.1.2 · repo `7c42aed` · hb5 `1428e55` |

**Closed in 14 days: 7** — latest:
...
- 2026-07-29 `wk-29bb6401` L-layout model: Run positions/direction + CornerLink + Kitchen leg adjacency emi…
